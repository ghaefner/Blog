    ...
    if form.validate_on_submit():
        ...
        if form.delete_picture.data:  # Check if delete picture button is clicked
            if current_user.image_file != 'default.jpg':
                picture_path = os.path.join(users.root_path, 'static/profile_pics', current_user.image_file)
...
def reset_request():
    """
    Render the page where users can input their email to request a password reset.

    Returns:
        render_template: Page template for users to input the email.
...
def reset_token(token):
    """
    Render the page users can reset their password.

    Args:
        token: generated token to verify authentication.

    Returns:
        render_template: Page template to input their new password.
